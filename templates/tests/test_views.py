...
class ViewTests(TestCase):
    ...
    def setUpTestData(cls):
        ...
        cls.user = User.objects.create_user(
            email="baws@baws.com", username="bawsbaws", password="tuplips901"
        )
        cls.mock_template_file: str = mock_macro_template()
        cls.tmpl = Template.objects.create(
            name="Test Template", source_file=cls.mock_template_file
        )
        cls.client = Client()
    ...
    def test_template_detail_basic(self):
        ...
        response = self.client.get(
            reverse("templates:template_detail", args=[self.tmpl.slug])
        )
        self.assertEqual(response.status_code, 200)
